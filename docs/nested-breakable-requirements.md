...
入れ子にした結果、左右の余白や枠線ぶん内側の本文幅が小さくなるのは自然な挙動である。
...
- `tcolorbox/`
  - 編集対象の `tcolorbox` 6.10.0 実行時ファイル
- `tests/a4/`
  - A4 用の、人間が読みやすいサンプル
- `tests/compare/`
...
- `build/compare/original/`
  - 元実装で生成した PDF と画像
- `build/compare/breakble/`
  - `breakble-tcolorbox` で生成した PDF と画像
- `build/compare/diff/`
  - 差分画像、比較レポート
...
少なくとも代表ケースについて、以下のエンジンで破綻しないことを確認する。
...
あるエンジン向けの調整が別エンジンの出力を壊してはいけない。
日本語を含む実ユースケースでは、LuaLaTeX と upLaTeX + dvipdfmx の両方で
文字化け、重なり、大きなページ下部余白が出ないことを確認する。

実装は紙面サイズを A4 固定で仮定してはならない。
現在のページ、本文領域、親 fragment の実際の残り高さから計算し、A4 以外の紙面でも動くこと。

また、全ページを何度も画像解析しながら組版するような重い方式は避ける。
検証では画像比較を使ってよいが、通常の文書コンパイル時の実装は TeX の箱分割処理の範囲で完結させる。
...
現在のリポジトリでは、次を基準に確認する。

1. `vendor/tcolorbox-original/` に upstream `tcolorbox` 6.10.0 の未改変実行時ファイルを置く。
2. `tcolorbox/` で改変版を編集する。
3. `scripts/build-safe-runtime-tree.py` で、同名衝突を避けた配布用ファイルを
   `texmf/tex/latex/breakble-tcolorbox/` と `drop-in/breakble-tcolorbox/` に生成する。
4. `docs/samples/` と `docs/readme-demo/` の A4 / 非A4 サンプルをコンパイルし、
   `verification/nested-behavior/pdf/` に目視確認用 PDF を置く。
5. `scripts/check-upstream-example-parity.py` で upstream の単体サンプルを pixel 比較する。
6. `scripts/check-upstream-manual-parity.py` で upstream manual 全体を pixel 比較する。
7. 入れ子なしの upstream example / manual は pixel 一致を合格条件にする。
8. 入れ子ありの独自サンプルでは、ページ上部の不自然な空白、ページ下部到達度、
   謎の色マーカー、本文と数式の重なり、`first/middle/last/unbroken` 系描画を合格条件として見る。

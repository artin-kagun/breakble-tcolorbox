...
- `verification/nested-behavior/pdf/a4-proof-environment-nested-overlap-uplatex.pdf`:
  日本語の Proof/SubProof 風の実使用例を upLaTeX で確認する PDF です。
- `verification/nested-behavior/pdf/a4-proof-environment-nested-overlap-xelatex.pdf`:
  同じ Proof/SubProof 風の実使用例を XeLaTeX で確認する PDF です。
- `verification/nested-behavior/pdf/a4-proof-environment-continuous-inner-break-pdftex-viewable.pdf`:
  一般的な PDF ビューアでも日本語が読めるようにした Proof/SubProof の実使用例です。一本の長い内側 `SubProof` を開いたままページをまたぐ挙動を確認できます。
...
現在の検証結果では、upstream example は 3/3 件でページ数一致・pixel 一致、upstream manual は 568/568 ページで pixel 一致しています。入れ子挙動の代表サンプルは pdfLaTeX / XeLaTeX / upLaTeX + dvipdfmx / LuaLaTeX でビルドできることを確認しています。
